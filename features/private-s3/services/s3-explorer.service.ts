...
const getS3Client = (config: S3Config): S3Client => {
  ...
  const endpoint =
    config.endpoint && !isCloudFrontUrl(config.endpoint) ? config.endpoint : undefined;
...
export const s3ExplorerService = {
  ...
  async uploadFileMultipart(params: {
    ...
    if (
      existingState &&
      existingState.key === params.key &&
      existingState.bucket === params.config.bucket
    ) {
      ...
      const existingParts = await params.listUploadParts(
        client,
   ...
   */
  async deleteItem(params: { config: S3Config; key: string }) {
