...
import { AuthUser } from "@/features/auth/stores/auth-store";
...
export const useFileActions = (file: S3File, user: AuthUser | null) => {
  ...
  const handleRename = async (newName: string, onSuccess: () => void) => {
    ...
    if (!isPrivateS3 && !user?.isPro) {
      toast({
        description:
          "Modifying files in Managed Storage is limited to Pro users. Please switch to Own S3 or upgrade.",
        variant: "destructive",
      });
    ...
    try {
      ...
      if (storageMode === "own-s3") {
        ...
        await s3ExplorerService.rename(
          config,
          file.bucketFileId || file.$id,
          newKey,
          metadata.metadata
        );
      } else {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (isPrivateS3) {
        ...
        url = await platformStorageService.getDownloadUrl(
          file.bucketFileId || file.$id || file.key || ""
        );
      }
