  ...
  return (
    <div className={totalSpace?.all !== undefined ? "dashboard-container" : "w-full"}>
          ...
          {isLoading ? (
            <>
              ...
              <ul className="dashboard-summary-list">
                {Array.from({ length: 4 }).map((_, i) => ( // Corrected map syntax
                  <li key={i}>
                    <SkeletonSummaryCard />
                  </li>
                ))}
            ...
            <>
              ...
              {usageSummary.length > 0 && (
                <ul className="dashboard-summary-list">
                  {usageSummary.map((summary) => ( // Corrected map syntax
                    <UsageSummaryCard key={summary.title} {...summary} />
                  ))}
                </ul>
              )}
            </>
      ...
      <section
        className={
          totalSpace?.all !== undefined
            ? "dashboard-recent-files dashboard-right-content"
            : "w-full"
        }
      >
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center px-4 py-16 text-center">
            <div className="mb-4 rounded-full bg-light-300 p-4">
              <svg
                className="size-12 text-light-200"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
                aria-hidden="true"
            ...
            <p className="caption text-light-200/70">
