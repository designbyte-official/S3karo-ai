...
export async function GET(request: NextRequest) {
  try {
    ...
    const storageUsed = subscription ? Number(subscription.storageUsed || 0) : totalSpace.used;

    const bandwidthLimit = subscription
      ? Number(subscription.bandwidthLimit || 10737418240)
      : 10737418240; // Default 10GB

    const bandwidthUsed = subscription ? Number(subscription.bandwidthUsed || 0) : 0;
